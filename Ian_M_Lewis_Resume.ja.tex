...
\NewDocumentCommand{\budoux}{O{\raggedright} m}{{%
  ...
  \writeorigfile{\origfile}{#2}%

  \ShellEscape{export LC_ALL=C.UTF-8; cat \origfile | ./.venv/bin/python ./run_budoux.py > \processedfile}%
  % #1%
...
\begin{list}{}{
    ...
    \begin{itemize}
        ...
        \item \textbf{SIPSS} --- \emph{テックリード/プロジェクトマネージャー}\\
            ...
            \budoux{大規模ヘアサロン向けPOSシステムSIPSSのバックエンドアーキテクチャ近代化および新製品開発を担当し、インフラおよびクライアントアプリケーション開発者の小規模チームを管理しました。}
